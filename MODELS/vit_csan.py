# --------------------------------------------------------
# ViT-CSaN with two optional CSaN-v1 variants
...
from timm.layers import PatchEmbed, DropPath, AttentionPoolLatent, RmsNorm, PatchDropout, SwiGLUPacked, \
    ...
    get_act_layer, get_norm_layer, LayerType, to_2tuple
...
class Mlp(nn.Module):
    ...
    def __init__(
        ...
        #self.bias = nn.Parameter(0. * torch.ones(out_features), requires_grad=True) # 1-way bias
        self.drop2 = nn.Dropout(drop_probs[1])
        
        self.dim = in_features
        
        # addititve
        self.act = act_layer() #fles_mamba(hidden_features, ratio=32, drop_path=0.) 
...
# csan v1 (on vit patched global attention, version1: bounded kappas)
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        u = self.u(x).view(B, N, self.num_heads, -1).transpose(1, 2) # b N g d -> b g N h 
        ...
        ind = qscore * u.sigmoid() # b g N h 
        ind = ind.transpose(1, 2).reshape(B, N, -1) # b g N h -> b N g h -> b N H 
        
        indm = self.reduce(ind) 
        indm = self.act(indm) 
        
        ka = self.ka(indm).view(B, N, self.num_heads, self.head_dim).transpose(1, 2) + 1. 
        kv = self.kv(indm).view(B, N, self.num_heads, self.head_dim).transpose(1, 2)
...
# csan v1 (on vit patched global attention, version1: bounded kappas)
# a little bit lighter and faster version
# shows similar performance with unbounded-kappa version, but may be more numerically stable for deeper models?
class Attention(nn.Module):
    fused_attn: Final[bool]
    ...
    def forward(self, x):
        ...
        ka = tanh(self.ka(indm)).view(B, N, self.num_heads, self.head_dim).transpose(1, 2) + 1. 
        kv = tanh(self.kv(indm)).view(B, N, self.num_heads, self.head_dim).transpose(1, 2)
        
        temp = tanh(self.temp(indm)).transpose(1, 2).unsqueeze(-1) + 1. # b N g -> b g N 1
        magk = tanh(self.magk(indm)).transpose(1, 2).unsqueeze(-2) + 1. # b N g -> b g 1 N 
...
# csan v1 (on vit patched global attention, version2: unbounded kappas)
class SoftGShift(nn.Module):
    def __init__(self, dim):
        super().__init__()
...
@register_model
def vit_base_slim_patch16_224(pretrained=False, **kwargs) -> VisionTransformer:
...
"""
@register_model
def vit_base_patch16_224_fles(pretrained=False, **kwargs) -> VisionTransformer:
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12)
    model = _create_vision_transformer('vit_base_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
"""

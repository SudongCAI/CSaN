# --------------------------------------------------------
# Hiera-CSaN with two optional CSaN-v1 variants
...
# csan v1 (on hiera local attention, version1: bounded kappas)
class MaskUnitAttention(nn.Module):
    """
    Computes either Mask Unit or Global Attention. Also is able to perform q pooling.
    ...
    def __init__(
        ...
        ratio = 8 # or 8 12
        threshold = 24 
        hidden_dim = dim_out//ratio if dim_out//ratio > threshold else threshold # num_heads=(3, 6, 12, 24) 
        
        self.u = nn.Linear(dim, dim_out//4) 
        
        self.reduce = nn.Linear(dim_out//4, hidden_dim) 
        self.act = nn.GELU() #SoftGShift(dim=hidden_dim) 
        
        self.ka = nn.Linear(hidden_dim, dim_out) 
        self.kv = nn.Linear(hidden_dim, dim_out) 
        self.magk = nn.Linear(hidden_dim, self.heads) 
        self.temp = nn.Linear(hidden_dim, self.heads) 
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        num_windows = (N // (self.q_stride * self.window_size)) if self.use_mask_unit_attn else 1 
        L = N // num_windows 
        qkv = self.qkv(x).reshape(B, L, num_windows, 3, self.heads, self.head_dim).permute(3, 0, 4, 2, 1, 5) # 3 B g W L d
        q, k, v = qkv.unbind(0)
        
        u = self.u(x).view(B, L, num_windows, self.heads, -1).permute(0, 3, 2, 1, 4) # B L W g h => B g W L h 
        
        if self.q_stride > 1:
            # Refer to Unroll to see how this performs a maxpool-Nd
            q = q.view(B, self.heads, num_windows, self.q_stride, -1, self.head_dim).amax(dim=3) 
            Lq = q.shape[-2] 
            u = u.view(B, self.heads, num_windows, self.q_stride, Lq, -1) # B g W s L h
            
            attn = (q * self.scale) @ k.transpose(-1, -2) # B g W L h @ B g W h L -> B g W L L 
                
            # scales compute 
            qscore = self.quasi_linear(attn, 0.25).mean(dim=-1, keepdim=True).unsqueeze(-3) # B g W 1 L 1 
            
            ind = qscore * u.sigmoid() # B g W s L h 
            ind = ind.permute(0, 2, 3, 4, 1, 5).reshape(B, num_windows, self.q_stride, Lq, -1) # B g W s L h -> B W s L g h -> B W s L H 
            
            indm = self.reduce(ind) # B W s L H 
            indm = self.act(indm) # B W s L H  
            
            indma = indm.mean(dim=-3) # B W s L H -> B W L H 
            indm = indm.view(B, num_windows, L, -1) # B W L H
            
            ka = tanh(self.ka(indma)).view(B, num_windows, Lq, self.heads, self.head_dim).permute(0, 3, 1, 2, 4) + 1. # B W L g d -> B g W L d
            kv = tanh(self.kv(indm)).view(B, num_windows, L, self.heads, self.head_dim).permute(0, 3, 1, 2, 4)      # B W L g d -> B g W L d
            
            temp = tanh(self.temp(indma)).permute(0, 3, 1, 2).unsqueeze(-1) + 1. # B W L g -> B g W L 1 
            magk = tanh(self.magk(indm)).permute(0, 3, 1, 2).unsqueeze(-2) + 1. # B W L g -> B g W 1 L 

            attn = self.softmax(attn * temp) * magk # B g W L L
            #attn = self.attn_drop(attn)
            
            x = attn @ v * ka + (v * kv).view(B, self.heads, num_windows, self.q_stride, Lq, self.head_dim).mean(dim=-3) # B g W s L d

        else: 
            attn = (q * self.scale) @ k.transpose(-1, -2) # B g W L h @ B g W h L -> B g W L L 
                
            # scales compute 
            qscore = self.quasi_linear(attn, 0.25).mean(dim=-1, keepdim=True) # B g W L 1 
            
            ind = qscore * u.sigmoid() # B g W L h 
            ind = ind.permute(0, 2, 3, 1, 4).reshape(B, num_windows, L, -1) # B g W L h -> B W L g h -> B W L H 
            
            indm = self.reduce(ind) # B W L H
            indm = self.act(indm) # B W L H 
            
            ka = tanh(self.ka(indm)).view(B, num_windows, L, self.heads, self.head_dim).permute(0, 3, 1, 2, 4) + 1. # B W L g d -> B g W L d
            kv = tanh(self.kv(indm)).view(B, num_windows, L, self.heads, self.head_dim).permute(0, 3, 1, 2, 4)      # B W L g d -> B g W L d
            
            temp = tanh(self.temp(indm)).permute(0, 3, 1, 2).unsqueeze(-1) + 1. # B W L g -> B g W L 1 
            magk = tanh(self.magk(indm)).permute(0, 3, 1, 2).unsqueeze(-2) + 1. # B W L g -> B g W 1 L 
        ...
        x = x.transpose(1, 3).reshape(B, -1, self.dim_out) # B g W L d -> B L W g d -> B N C 
        x = self.proj(x)
        return x
...
# csan v1 (on hiera local attention, version2: unbounded kappas)
...
class MaskUnitAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.q_stride > 1:
            ...
            ka = self.ka(indma).view(B, num_windows, Lq, self.heads, self.head_dim).permute(0, 3, 1, 2, 4) + 1. # B W L g d -> B g W L d
            kv = self.kv(indm).view(B, num_windows, L, self.heads, self.head_dim).permute(0, 3, 1, 2, 4)      # B W L g d -> B g W L d
            
            temp = self.temp(indma).permute(0, 3, 1, 2).unsqueeze(-1) + 1. # B W L g -> B g W L 1 
            magk = self.magk(indm).permute(0, 3, 1, 2).unsqueeze(-2) + 1. # B W L g -> B g W 1 L 
            ...
            ka = self.ka(indm).view(B, num_windows, L, self.heads, self.head_dim).permute(0, 3, 1, 2, 4) + 1. # B W L g d -> B g W L d
            kv = self.kv(indm).view(B, num_windows, L, self.heads, self.head_dim).permute(0, 3, 1, 2, 4)      # B W L g d -> B g W L d
            
            temp = self.temp(indm).permute(0, 3, 1, 2).unsqueeze(-1) + 1. # B W L g -> B g W L 1 
            magk = self.magk(indm).permute(0, 3, 1, 2).unsqueeze(-2) + 1. # B W L g -> B g W 1 L 
...
class HieraBlock(nn.Module):
    def __init__(
        ...
        # unsync dp to preserve the use of game-scale scheme 
        if drop_path > 0.2:
            self.drop_path1 = DropPath(drop_path * 0.75) 
        else:
            self.drop_path1 = DropPath(drop_path) if drop_path > 0. else nn.Identity() 
...
@register_model
def hiera_tiny_plus_224(pretrained=False, **kwargs):
    model_args = dict(embed_dim=96, num_heads=3, stages=(1, 2, 7, 2))
    return _create_hiera('hiera_tiny_224', pretrained=pretrained, **dict(model_args, **kwargs))
...
@register_model
def hiera_base_slim_224(pretrained=False, **kwargs):
    model_args = dict(embed_dim=72, num_heads=1, stages=(2, 3, 16, 3))

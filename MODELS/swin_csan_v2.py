# --------------------------------------------------------
# Swin-CSaN-v2 with two optional CSaN variants
...
# csan v2 (on swin window attention, version1: bounded kappas)
# Compared with v1, v2 adds a DWConv-based posinfo module to enable
# structure-aware integration of CSaN's refined non-local cues.
# To better fit Swin's block organization, posinfo is moved into
# SwinTransformerBlock as a post-Attn-CSaN refinement module.
...
# csan v2 (on swin window attention, version2: unbounded kappas)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.posinfo = nn.Conv2d(dim, dim, kernel_size=7, padding=3, groups=dim) # depthwise conv 
        #self.posinfo = nn.Conv2d(dim, dim, kernel_size=5, padding=2, groups=dim) # depthwise conv 
        #self.posinfo = self.posinfo.to(memory_format=torch.channels_last)
        ...
        # ./3 swin-small:   non-ema: 83.35x     ema: 83.50x
        if drop_path < 0.25:
        ...
        #self.act1 = nonlinearity(dim)
        #self.act2 = nonlinearity(dim)
    ...
    def forward(self, x):
        ...
        shortcut = x
        x = self.norm1(x)
        #x = self.act1(self.norm1(x), x) #self.norm1(x)
        ...
        #shifted_x = window_reverse_nchw(attn_windows, self.window_size, H, W).contiguous(memory_format=torch.channels_last)  # B C H' W'
        
        # structural info recall
        shifted_x = shifted_x.permute(0, 3, 1, 2).contiguous(memory_format=torch.channels_last) # (B, H, W, C) -> (B, C, H, W) 
        shifted_x = self.posinfo(shifted_x) 
        shifted_x = shifted_x.permute(0, 2, 3, 1) #.contiguous() # (B, C, H, W) -> (B, H, W, C) 
        ...
        #x = x.reshape(B, -1, C)
        #x = x + self.drop_path(self.mlp(self.act2(self.norm2(x), x)))
...
@register_model
def swin_min_csan_v2(pretrained=False, **kwargs) -> SwinTransformer:
...
@register_model
def swin_micro_csan_v2(pretrained=False, **kwargs) -> SwinTransformer:
    """ @ 224x224, trained ImageNet-1k
    """
...
@register_model
def swin_tiny_slim_csan_v2(pretrained=False, **kwargs) -> SwinTransformer:
    """ @ 224x224, trained ImageNet-1k
    """
...
@register_model
def swin_tiny_csan_v2(pretrained=False, **kwargs) -> SwinTransformer:
    """ @ 224x224, trained ImageNet-1k
    """
...
@register_model
def swin_small_csan_v2(pretrained=False, **kwargs) -> SwinTransformer:
    """ @ 224x224
    """
...
@register_model
def swin_base_csan_v2(pretrained=False, **kwargs) -> SwinTransformer:
    """ @ 224x224
    """
